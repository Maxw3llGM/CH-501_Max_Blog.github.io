...
public class Slingshot : MonoBehaviour
{
    ...
    private void Gravity()
    {
        ...
        for (int i = 0; i < celestials.Length; i++)
        {
            ...
            //Debug.Log(main_planet);
            Vector2 accel = Vector2.zero;
            planet_vel[i] += accel * physicsTimeStep;

            foreach (GameObject sub_planet in celestials)
            {
                if (!main_planet.Equals(sub_planet))
                {
                    float sub_mass = sub_planet.GetComponent<Rigidbody2D>().mass;
        ...
        for (int i = 0; i < celestials.Length; i++)
        {
            ...
            celestials[i].GetComponent<Rigidbody2D>().MovePosition(celestials[i].GetComponent<Rigidbody2D>().position + planet_vel[i] * physicsTimeStep);
