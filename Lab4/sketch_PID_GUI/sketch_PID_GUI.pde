...
void setup(){
  ...
  //cp5.addButton("Random Refresh rate")
  //   .setValue(0)
  //   .setPosition(10,620)
  //   .setSize(200,50)
  //   ;
...
while(1==1) {
 ...
 // Torques from difference in endeffector and setpoint, set gain, calculate force
      ...
      if (iter1 > 10){
        ...
        if (abs(dist_X) < 0.5) cumerrorx = 0;`
        if (abs(dist_Y) < 0.5) cumerrory = 0;
      }
    ...
    // Forces are constrained to avoid moving too fast
      ...
      //println("Time Difference: ", timedif, " | cumerrorx =",cumerrory, " | distx = ", dist_X, " | diffx = ", diffx);
      if(noforce==1)
      {
        fEE.x=0.0;
        fEE.y=0.0;
